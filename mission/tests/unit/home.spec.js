...
describe('Home.vue', () => {
  ...
  // text 데이터 검사 - 실패
  test('실패:: text가 빈값이면 isText는 true가 아닙니다.', () => {
    wrapper.vm.text = '';
  ...
  // text 데이터 업데이트 - 실패
  test('실패:: text에 값이 없으면 text 데이터를 업데이트 하지 않습니다.', () => {
  ...
  // text 순서 회전 - 실패
  test('실패:: shakeText 함수가 실행되면 text의 순서는 2칸씩 회전하지 않습니다.', () => {
  ...
  // 클릭 카운터 업데이트 - 실패
  test('실패:: 클릭 횟수는 데이터가 유효하지 않으면 증가하지 않습니다.', () => {
